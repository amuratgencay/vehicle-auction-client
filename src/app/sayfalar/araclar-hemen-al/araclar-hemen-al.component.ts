...
import {
  IhaleAracListeDTO, HemenAlListeDTO, AracAraDTO,
  IhaleAracService, HemenAlService, IAracAraDTO, IhaleTipi
} from '@zyazilim/online-ihale-data';

@Component({
  selector: 'app-araclar-hemen-al',
  templateUrl: './araclar-hemen-al.component.html',
  styleUrls: ['./araclar-hemen-al.component.scss']
})
export class AraclarHemenAlComponent implements OnInit, OnDestroy {
  aramaSonucu: IhaleAracListeDTO[];
  ihale: HemenAlListeDTO;
  tab: string;
  listeTipi = 'card';
  ...
  constructor(
    ...
    private ihaleService: HemenAlService,
    ...
    this.eventSub$ = this.router.events.pipe(filter(event => event instanceof NavigationEnd))
      .subscribe(event => {
        ...
        this.tab = params.tab;
        if (params.tab === 'aktif') {
          this.ihaleService.getAktifIhale().subscribe(d => {
            this.ihale = d;
            if (!this.ihale) {
  ...
  private getData() {
    ...
    this.aramaKriter.ihaleTipi = IhaleTipi.HemenAl;
